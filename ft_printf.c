...
static	int	ft_format_check(va_list *arg, char format)
{
	if (format == 'c')
		...
		return (ft_hex_base(va_arg((*arg), unsigned long int),
				"0123456789abcdef", -1));
		...
		return (ft_hex_base(va_arg((*arg), unsigned int),
				"0123456789ABCDEF", 0));
	else if (format == 'x')
		return (ft_hex_base(va_arg((*arg), unsigned int),
				"0123456789abcdef", 0));
...
static	int	ft_print(va_list *arg, const char *str)
{
	...
	while (str[++i])
	{
		if (str[i] == '%')
		{
			de_len = ft_format_check(arg, str[++i]);
			if (de_len == -1)
				return (-1);
			len += de_len - 1;
		}
